...
export default function BottomNav() {
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-20 border-t border-(--color-graphite-200) bg-white/92 backdrop-blur-md"
        ...
        {items.map((item) => (
          <li key={item.to} className="flex-1">
            <NavLink
              ...
              className={({ isActive }) =>
                `tap flex flex-col items-center gap-1 py-2 text-xs font-semibold active:scale-[0.96] ${
            ...
            >
              {({ isActive }) => (
                <>
                  <span
                    className={`tap grid h-11 w-11 place-items-center rounded-2xl ${
                      isActive ? 'bg-(--color-leaf-100)' : ''
                    }`}
                  >
